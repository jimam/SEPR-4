...
public class Level implements Screen {
	...
	/**
	 * Spawns multiple zombies cycling through spawnPoints until the given amount
	 * have been spawned.
	 *
	 * @param spawnPoints
	 *            locations where zombies should be spawned on this stage
	 * @param amount
	...
	public int spawnZombies(int amount, ArrayList<Vector2> spawnPoints) {
		...
		for (int i = 0; i < amount; i++) {
			...
			switch (chooseType) {
			case 0:
				zombie = (new Zombie(new Sprite(new Texture("zombie01.png")), spawnPoints.get(i % spawnPoints.size()),
						this, Zombie.Type.SLOW));
				break;
			case 1:
				zombie = (new Zombie(new Sprite(new Texture("zombie02.png")), spawnPoints.get(i % spawnPoints.size()),
						this, Zombie.Type.MEDIUM));
				break;
			case 2:
				zombie = (new Zombie(new Sprite(new Texture("zombie03.png")), spawnPoints.get(i % spawnPoints.size()),
						this, Zombie.Type.FAST));
				break;
			default:
				zombie = (new Zombie(new Sprite(new Texture("zombie02.png")), spawnPoints.get(i % spawnPoints.size()),
						this, Zombie.Type.MEDIUM));
	...
	public boolean isBlocked(float x, float y) {
		...
		// we have to add the property now because the properties don't load from the
		// map file
	...
	/**
	 * Converts the mousePosition which is a Vector2 representing the coordinates of
	 * the mouse within the game window to a Vector2 of the equivalent coordinates
	 * in the world.
	 *
	 * @return Vector2 of the mouse position in the world.
	 */
	public Vector2 getMouseWorldCoordinates() {
		// Must first convert to 3D vector as camera.unproject() does not take 2D
		// vectors.
	...
	public void show() {
		...
		// It is only possible to view the render of the map through an orthographic
		// camera.
	...
	public void render(float delta) {
		if (isPaused) {
			...
			// Try to spawn all zombie in the stage and update zombiesToSpawn with the
			// amount that failed to spawn
			...
			for (int i = 0; i < aliveZombies.size(); i++) {
				...
				// Zombies will only attack if they are in range, the attack has cooled down,
				// and they are
				// facing a player.
				// Player will only attack in the reverse situation but player.attack must also
				// be true. This is
				// controlled by the ZeprInputProcessor. So the player will only attack when the
				// user clicks.
				if (player.attack) {
			...
			if (currentPowerUp != null) {
				// Activate the powerup up if the player moves over it and it's not already
				// active
			...
			String progressString = ("Wave " + Integer.toString(currentWave) + ", " + Integer.toString(zombiesRemaining)
					+ " zombies remaining.");
